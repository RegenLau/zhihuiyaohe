...
<script setup lang="ts">
  ...
  const breadcrumbItems = computed<BreadcrumbItem[]>(() => {
    const { matched } = route
    const visibleMatched = matched.filter((item) => !item.meta?.isHide)
    const matchedLength = visibleMatched.length

    // 处理首页情况
    if (!matchedLength || isHomeRoute(visibleMatched[0])) {
      return []
    }

    // 处理一级菜单和普通路由
    const firstRoute = visibleMatched[0]
    const isFirstLevel = firstRoute.meta?.isFirstLevel
    const lastIndex = matchedLength - 1
    const currentRoute = visibleMatched[lastIndex]
    const currentRouteMeta = currentRoute.meta

    let items = isFirstLevel
      ? [createBreadcrumbItem(currentRoute)]
      : visibleMatched.map(createBreadcrumbItem)
